...
export default function InterviewPage() {
  ...
  return (
    <div className="max-w-3xl mx-auto px-4 py-8 animate-fade-in">
      ...
      <Card className="p-8 mb-6">
        <div className="flex items-center gap-3 mb-4">
          <span className={`
            ...
            ${currentQuestion?.type === 'technical' ? 'bg-blue-500/10 text-blue-400' :
              currentQuestion?.type === 'behavioral' ? 'bg-purple-500/10 text-purple-400' :
              'bg-amber-500/10 text-amber-400'}
          `}>
            {currentQuestion?.type === 'technical' ? 'Technique' :
             currentQuestion?.type === 'behavioral' ? 'Comportementale' : 'Situationnelle'}
          </span>
        </div>
      ...
      {/* Feedback */}
      {feedback && (
        <Card className={`p-6 mb-6 ${feedback.answer_score >= 70 ? 'border-green-500/30' : 'border-amber-500/30'}`}>
          <div className="flex items-center gap-3 mb-3">
            ...
            <span className={`text-lg font-bold ${feedback.answer_score >= 70 ? 'text-green-400' : 'text-amber-400'}`}>
